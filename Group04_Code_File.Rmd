...
```{r}
# Firstly, we will run a model with all variables.

model00 <- lm(AbsentHours ~., data = emp)


##################

# We will do our correlation matrix, but since our data contain numirc and nonnumirc variable, we will do the correlation matrix manually.
...
# After computing the correlation between our response variable and other variables, we will do tests of the correlations between some variables to test our assumptions of having multycolinearity between some variables.

# First test will be between BusinessUnit and Division.

cor(m_V6, m_V7) 

# As we can see, we have a significant 100% correlation between DivisionStores and (BusinessUnitHeadOffice and BusinessUnitStores), which is indecates that the presence of the two variables in our model will result in a multyculinearity issues.
...
# Based on the optained results, we can conclude that the precense of these two variables will create a problem of multicolinearity, due to the high correlation (more than95%) between some of the observations.

# Therefore, our optimal solution is to remove one of the two variables.


#############
# For the other variables, there is  no need for testing the correlation between them, due to that it seems that there is no significant correlation between them, but if a high correlation was precent, R will find it and solve the problem.
...
# We have deleted the listed variables to impelement our first model because of the below reasons:


# DepartmentName: Becuase it is related to the JubTitle, therefore, if we include it in the model a multicolieanerity problem will rise.

# Division: Becuase it is related to the BusinessUnit, therefore, if we include it in the model a multicolieanerity problem will rise.


model01 <- lm(AbsentHours ~., data = PredData)
anova(model01)

# According to our ANOVA table it is clear that all of our variables are significant based on P-value test approach.
...
Model02 <- lm(AbsentHours ~., data = BackwardData)

Model02 <- step(Model02, direction = "backward", trace=FALSE ) 

#Now, we will view the results of Backward elemination regression

Model02$anova

#Finally, we will view our final model of backward elemination method.
...
model04 <- lm(AbsentHours ~ ., data=Stipwisedata)


# Finally, we wil perform forward stepwise regression

forwardModel <- step(intercept_only, direction='forward', scope=formula(model04), trace=0)
...
forwardModel$coefficients


##################################################

# As we can see, using these two approaches (Backward elemination, Forward stepwise), we got the same results!. Therefore, our optimal model will be with the coffecients given below:
