...
Here we will try to build our model using AbsentHours to be our dependent variable.
...
```
 ...
 As we can obsirve, R-squared for the full model came out to be 0.7407666


After computing the models, we will create a new dataset to implement our First model based on the variables that we beleive it has an impact on our respond variable.



*(PredData)* refer to the data that we are going to use in building our new model.
...
According to the results optained using this model, it shows that there is a variable (EmployeeNumber) which is insignificant at alpha = 0.05, which result in some NA values.
...
*BackwardModel* represent our model using backward elemination method
...
As we can see, after using Backward Elemination method, we have define six variables to be in our model and all of them are significant at alpha = 0.05
...
3- we wil perform forward stepwise regression
...
Using forward Stepwise method, we have obtained five variables to be used for our model, but one of these variables (LengthService) is insignificant at alpha = 0.05, therefore, it will be dropped from the model and use just the other four variables which are (Age, Gender, AbsHrsPerTenure, JobTitl).
...
After deleting the insignificant variable, the model become significant and can be used in our regression.

R-squared is 0.7391996


As we can see, using these two approaches (Backward elemination, Forward stepwise), we got the same results for the R-squared approach!. Therefore, our optimal model using (AbsentHours) as a response variable will be the model that was obtained using Forward stepwise approach, due to that it has lower number of variables than Backward Elimination approach with the same coffetiont of determinition.
...
From the obtained result we can see that the variable that has the highest impact on response variable is *AbsentHours* then *Age*.
...
After computing the models, we will create a new dataset to implement our First model based on the variables that we beleive they have a high impact on our respond variable

*(PredData2)* refer to the data that we are going to use in building our new model.
...
According to the results optained using this model, it shows that there are some variables that are insignificant, As a result, some observasions are having NA values, to reduce them to minimum, we will try some advanced methods of modeling.
...
*BackwardData2* represent the data that we are going to use to impelemente our model using backward elemination method
...
BackwardModel2$coefficients
summary(BackwardModel2)$r.squared
```

As we can see, after using Backward Elemination method, our observations has been reduce to four, also, one of these for is insignificant (JobTitle) based on P-value approach at alpha = 0.05.

Also, R-squared is found out to be 0.2362546
...
*Stipwisedata2* represent the data that we are going to use to impelemente our model using Forward stepwise method.
...
#Secondly, we will define a model with all predictors
# * We have defined it before with the name FullModel
# Finally, we wil perform forward stepwise regression
StepwiseModel2 <- step(intercept_only2, direction='forward', scope=formula(FullModel), trace=0)
#Now, we will view the results of forward stepwise regression
...
rm(PredData, PredData2, Stipwisedata, Stipwisedata2,BackwardData,BackwardData2)
     
```

As we can see, we have define in this model 4 variables to be added to our model, and all of them are significant at alpha = 0.05.

Also, R-squared came out to be 0.2321445

--------------------------------------------------------------------------

As we can see, using these two approaches (Backward elemination, Forward stepwise), we gotdefferint results. Furthurmore, coffetion of determinition is very small for the two method. Therefore, our optimal model will be either Backward elemination with *AbsentHours* as a response variable or Forward stepwise with *AbsentHours* as a response variable.
